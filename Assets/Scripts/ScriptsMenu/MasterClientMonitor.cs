...
namespace FirstGearGames.PhotonNetworking.Utilities
{
    ...
    public class MasterClientMonitor : MonoBehaviourPunCallbacks
    {
        ...
        {
            public PlayerPing(Photon.Realtime.Player player, int ping)
            {
                Player = player;
                _pings.Add(ping);
            }
            ...
            /// </summary>
            public readonly Photon.Realtime.Player Player;
            /// <summary>
        ...
        private void SetNewMaster(Photon.Realtime.Player newMaster, bool resetHighPingCount = true)
        {
            if (resetHighPingCount)
                _consequtiveHighPingCount = 0;
            //Set new master and use takeover.
            PhotonNetwork.SetMasterClient(newMaster);
            Debug.Log(newMaster.NickName);
        }
        ...
        [PunRPC]
        private void RPC_ReceivePing(Photon.Realtime.Player player, int ping)
        {
        ...
        [PunRPC]
        private void RPC_RequestMasterClient(Photon.Realtime.Player requestor)
        {
