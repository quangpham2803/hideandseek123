...
namespace CoolBattleRoyaleZone
{
	...
	public class Zone : Singleton < Zone >
	{
		...
		private void Start ( )
		{
            ...
            {
                photonView.RPC("CreateEvents", RpcTarget.AllBuffered);
                // Creating events
                if (ResetupZoneCirclesOnStart)
                {
                    int i = Random.Range(0, FirstZoneCircle.Length);
                    int j = Random.Range(0, LastZoneCircle.Length);
                    photonView.RPC("SetupCircles", RpcTarget.AllBuffered, i, j);
                }  // Setuping circles if needed
                if (StartupTheZoneOnStart)
                {
                    photonView.RPC("WaitForNewSafeZoneCmd", RpcTarget.AllBuffered, ZoneCircles[0].StartDelay, CurStep);
		...
		public void SetupCircles (int firstCircleId, int lastCircleId)
		{
            ...
            tm.y = 5;
            DoorGame.door.escape.transform.position = tm;
             // Then reseting his y level to zone y level
			...
			if ( ZoneCircles != null )
			{
				for ( var i = 0; i < ZoneCircles.Count; i ++ )
				{
					if (i == 2)
                    {
                        ZoneCircles[i].ShrinkingTime = ConfigDataGame.shrink3;
                        ZoneCircles[i].StartDelay = ConfigDataGame.delay3;
                    }
                    else if( i == 3)
                    {
                        ZoneCircles[i].ShrinkingTime = ConfigDataGame.shrink4;
                        ZoneCircles[i].StartDelay = ConfigDataGame.delay4;
                    }
                    else if(i == 4)
                    {
                        ZoneCircles[i].ShrinkingTime = ConfigDataGame.shrink4;
                    ...
                    {
                        ZoneCircles[i].ShrinkingTime = ConfigDataGame.shrink1;
                        ZoneCircles[i].StartDelay = ConfigDataGame.delay1;
                    }
        ...
        public IEnumerator ChangeRadius(Vector3 position, float radius, float timeToChange)
        {
            ...
            if (CurStep != StepsToEnd - 1 && PhotonNetwork.IsMasterClient)
            {
                photonView.RPC("WaitForNewSafeZoneCmd", RpcTarget.AllBuffered, ZoneCircles[CurStep].StartDelay, CurStep);
        ...
        public IEnumerator WaitForNewSafeZone ( float timeToMove)
		{
            ...
            if (PhotonNetwork.IsMasterClient)
            {
                photonView.RPC("ChangeRadiusCmd", RpcTarget.AllBuffered, ZoneCircles[step].PositionAndRadius.Position,
