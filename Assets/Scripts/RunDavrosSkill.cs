...
using Photon.Pun;
public class RunDavrosSkill : MonoBehaviour
{
    ...
    public void UICountDown()
    {
        ...
        else
        {
            ...
            if (time >= 0.3)
            {
                ...
                if (isFirstTime == true)
                {
                    int timeTemp = (int)(ConfigDataGame.runSkill - time + 1);
                    backgroundCountDown.gameObject.SetActive(true);
                    countDownText.text = timeTemp.ToString();
                    countdownUI.fillAmount = time / ConfigDataGame.runSkill;
                }
                else
                {
    ...
    public void Use()
    {
        rpc.photonView.RPC("FastRunDavrosSkill", RpcTarget.AllBuffered, true);
        //rpc.StartCoroutine(rpc.RunDavros());
    }
